...
class EmployeeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (\Auth::user()->can('manage employee')) {
            ...
            {
                $employees = Employee::with(['designation', 'branch', 'department'])->orderBy('employee_id','desc')->get();
        ...
        if (\Auth::user()->can('create employee')) {
            $validator = \Validator::make(
                ...
                [
                    ...
                    'dob' => 'required|date|after_or_equal:1950-11-01|before_or_equal:2006-01-01',
                    'phone' => ['required','regex:/^\d{10,12}$/'],
                    'address' => ['required', 'max:255'],
                    'email' => ['required', 'unique:users', 'regex:/^[\w\-\.]+@[a-zA-Z0-9\-]+(\.[a-zA-Z]{2,})+$/'],
                    'password' => ['required', 'regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*[@_\-])(?=.*[0-9])[A-Za-z\d@_\-]{8,}$/'],
                    'branch_id' => 'required',
                ...
                [
                    ...
                    'name.string' => __('Name should contain only alphabetic characters.'),
                ],
                [
                    'phone.required' => __('Phone number is required.'),
                    'phone.regex' => __('Phone number should be proper format'),

                ],
                [
                    'email.required' => __('Email-ID is required.'),
                    'email.regex' => __('Email  should be in a proper way.'),

                ],
                [
                    'password.required' => __('Password is required.'),
                    'password.regex' => __('Password should contain at least one uppercase letter, one lowercase letter, one special character (@_-), one digit, and be at least 8 characters long.'),
                    // Other custom error messages...
                ],
                [
                    'address.required' => _('address is required'),
                    'address.max' => __('Address should not exceed :max characters.'),
                ],
                [
                    'dob.required' => __('Date of birth is required.'),
                    'dob.date' => __('Please provide a valid date for the date of birth.'),
                    'dob.after_or_equals' => __('Date of birth should be after or equal to :date.'),
                    'dob.before_or_equals' => __('Date of birth should be before or equal to :date.'),
                    // Other custom error messages...
                ]
            ...
            //if ($total_employee < $plan->max_users || $plan->max_users == -1) {
                $user = User::create(
                    [
                        'name' => $request['salutation'].' '.$request['name'],
                        'email' => $request['email'],
    ...
    public function update(Request $request, $id)
    {
        ...
        if (\Auth::user()->can('edit employee')) {
            ...
            $set_salary=Employee::set_salary($request->annual_salary,$id);
            $CTC=$request->annual_salary;
        if((($CTC*25)/100) > 180000){
           ...
           // $employee->fill($input)->save();
            $employee->fill($input);
            $employee->salary=$MonthlybasicSal;
            ...
            if (!empty($user)) {
                $user->name = $request->salutation.' '.$employee->name;
    ...
    public function dept_json(Request $request)
    {
    ...
    {
        $latest = Employee::orderBy('employee_id','desc')->first();
        if (!$latest) {
            return 1;
        }
    ...
    public function offerletterPdf($id)
    {
        ...
        //   // Total CTC
        //   $totalCTC = $basicSal + $HRA + $fixedAllow;
        ...
        //   $roundedGratuity = round($gratuity);
        //   $totalDeduction = $PF1 + $PF2 + $PT + $insurance + $roundedGratuity;
    
        //   $netInHand = $totalCTC - $totalDeduction;
        //   $monthlyTotalDed = ($PF1/12)+($PF2/12)+(($PT / 12) - 8)+($insurance / 12)+($roundedGratuity/12);
        //   $monthlyInhand = ($totalCTC/12) - $monthlyTotalDed;
        //   $structure=[
        //     'basicSal'=>$basicSal,
        //     'HRA'=>$HRA,
        ...
        //     'netInHand'=>$netInHand,
        //     'monthlyTotalDed'=>$monthlyTotalDed,
        ...
        $obj = [
            ...
            'designation' => !empty($employees->designation) ? $employees->designation : '',
            'start_date' => !empty($employees->joining_date) ? $employees->joining_date : '',
        ...
        return view('employee.template.offerletterpdf', compact('obj', 'employees','structure'));
    }
    ...
    public function RelLetterPdf($id)
    {
        $resignation=Resignation::where('id',$id)->first();
        $employees=Employee::find($resignation->employee_id);
        $date = date('Y-m-d');
        $settings = Utility::settings();
        $secs = strtotime($settings['company_start_time']) - strtotime("00:00");
        $result = date("H:i", strtotime($settings['company_end_time']) - $secs);
        $date1 = date_create($employees->company_doj);
        $date2 = date_create($resignation->resignation_date);
        ...
        return view('employee.template.RelLertterPdf', compact('obj', 'employees'));
    }
    public function ExpCertificatePdf($id)
    {
        ...
        $result = date("H:i", strtotime($settings['company_end_time']) - $secs);
            ...
            $date1 = new DateTime($employees->company_doj); // Start date
            $date2 = new DateTime($resignation->resignation_date); // End date
            ...
            // Adjust for the specific case from June 1 to August 31
            if ($date1->format('d') == 1 && $date2->format('d') == 31 || $date2->format('d') == 30) {
                $months++; // Add a month if starting from the first and ending on the last day of the month
            }

            // If the number of months is 12, increment the year and reset months to 0
            if ($months >= 12) {
      ...
      //  $experience_certificate->content = ExperienceCertificate::replaceVariable($experience_certificate->content, $obj);
        return view('employee.template.ExpCertificatepdf', compact('obj', 'employees'));
    }
    public function WorkingletterPdf($id)
    {
        $users = \Auth::user();
       ...
       /// $joiningletter->content = JoiningLetter::replaceVariable($joiningletter->content, $obj);
        return view('employee.template.WorkingLetterpdf', compact('obj', 'employees'));
    }
      // Increment Letter
      public function IncrementLetterPdf($id)
      {
          ...
          $date = date('Y-m-d');
           ...
           $increament = Appraisal::find($id);
           $employees=Employee::find($increament->employee);
           $structure=$this->annaxture($increament->appraisal_salary);
         ...
         /// $joiningletter->content = JoiningLetter::replaceVariable($joiningletter->content, $obj);
          return view('employee.template.IncrementLetterpdf', compact('obj', 'employees','structure','increament'));
      }
      public function salarySlipPdf($id)
    {
        $users = \Auth::user();
       ...
       /// $joiningletter->content = JoiningLetter::replaceVariable($joiningletter->content, $obj);
        return view('employee.template.SalarySlippdf', compact('obj', 'employees'));
    }
    public function annaxture($CTC)
    {
        ...
        $PF1 = 21600;
          ...
          $netInHand = $totalCTC - $totalDeduction;
          $monthlyTotalDed = ($PF1/12)+($PF2/12)+($PT / 12)+($insurance / 12)+($roundedGratuity/12);
          $monthlyInhand = ($totalCTC/12) - $monthlyTotalDed;
          $structure=[
            'basicSal'=>$basicSal,
            'HRA'=>$HRA,
    ...
    public function appraisalPdf($id)
    {
        $users = \Auth::user();
        ...
        $employees=Employee::find($appraisal->employee);
        $settings = Utility::settings();
        $secs = strtotime($settings['company_start_time']) - strtotime("00:00");
        $result = date("H:i", strtotime($settings['company_end_time']) - $secs);
        $structure=$this->annaxture($appraisal->appraisal_salary);
        ...
        return view('employee.template.appraisal_Letter_pdf', compact( 'employees','obj','appraisal','structure'));
    }
    ...
    public function appointmentPdf($id)
    {
        ...
        return view('employee.template.appointmentpdf', compact('structure', 'employees','obj'));
    }
