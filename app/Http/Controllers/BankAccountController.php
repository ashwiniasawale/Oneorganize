...
class BankAccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create bank account'))
        {
            ...
            //     return redirect()->route('bank-account.index')->withErrors( $validator)->with($messages)->first();
            // }
// Custom message
...
if ($validator->fails()) {
    $messages = $validator->getMessageBag();
    // Add custom message to message bag
 // $msg =  $messages->add('custom_message', $customMessage);
    return redirect()->route('bank-account.index')->with('error', __($customMessage))->withErrors($messages);
}
