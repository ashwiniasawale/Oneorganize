...
class ProposalController extends Controller
{
    ...
    public function create($customerId)
    {
        if(\Auth::user()->can('create proposal'))
        {
            ...
            return view('proposal.create', compact('customers', 'proposal_number', 'product_services', 'category', 'customerId'));
        }
        else
        {
...
//            $proposal->discount_apply = isset($request->discount_apply) ? 1 : 0;
            ...
            for($i = 0; $i < count($products); $i++)
            {
                ...
                $proposalProduct->tax         = $products[$i]['itemTaxPrice'];
//                $proposalProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function edit($ids)
    {
        ...
        if(\Auth::user()->can('edit proposal'))
        {
            ...
            return view('proposal.edit', compact('customers', 'product_services', 'proposal', 'proposal_number', 'category', 'items'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
