...
//        dd($request->all());
    ...
    public function leave(Request $request)
    {
        ...
        if (\Auth::user()->can('manage report')) {
            ...
            $employees = $employees->where('is_active','=','1')->get();
            ...
            foreach ($employees as $employee) {
                ...
                ->join('leave_mappings','leave_mappings.leave_id','=','leaves.id')->where('leave_mappings.status', 'Approved');
                $reject = Leave::where('employee_id', $employee->id)
                ->join('leave_mappings','leave_mappings.leave_id','=','leaves.id')->where('leave_mappings.status', 'Reject');
                $pending = Leave::where('employee_id', $employee->id)
                ->join('leave_mappings','leave_mappings.leave_id','=','leaves.id')->where('leave_mappings.status', 'Pending');

                if ($request->type == 'monthly' && !empty($request->month)) {
                    $month = date('m', strtotime($request->month));
                    $year = date('Y', strtotime($request->month));

                    $approved->whereMonth('leave_mappings.leave_date', $month)->whereYear('leave_mappings.leave_date', $year);
                    $reject->whereMonth('leave_mappings.leave_date', $month)->whereYear('leave_mappings.leave_date', $year);
                    $pending->whereMonth('leave_mappings.leave_date', $month)->whereYear('leave_mappings.leave_date', $year);
                ...
                if ($request->type == 'yearly' && !empty($request->year)) {
                    $approved->whereYear('leave_mappings.leave_date', $request->year);
                    $reject->whereYear('leave_mappings.leave_date', $request->year);
                    $pending->whereYear('leave_mappings.leave_date', $request->year);
    ...
    public function employeeLeave(Request $request, $employee_id, $status, $type, $month, $year)
    {
        if (\Auth::user()->can('manage report')) {
            ...
            foreach ($leaveTypes as $leaveType) {
                ...
                $totalLeave = Leave::where('employee_id', $employee_id)
                ->join('leave_mappings','leave_mappings.leave_id','=','leaves.id')
                ->where('leave_mappings.status', $status);
                if ($type == 'yearly') {
                    $totalLeave->whereYear('leave_mappings.leave_date', $year);
                } else {
                    $m = date('m', strtotime($month));
                    $y = date('Y', strtotime($month));

                    $totalLeave->whereMonth('leave_mappings.leave_date', $m)->whereYear('leave_mappings.leave_date', $y);
                }
            ...
            $leaveData = Leave::where('employee_id', $employee_id)
            ->join('leave_mappings','leave_mappings.leave_id','=','leaves.id')
            ->where('leave_mappings.status', $status);
            if ($type == 'yearly') {
                $leaveData->whereYear('leave_mappings.leave_date', $year);
            } else {
                $m = date('m', strtotime($month));
                $y = date('Y', strtotime($month));

                $leaveData->whereMonth('leave_mappings.leave_date', $m)->whereYear('leave_mappings.leave_date', $y);
            }
    ...
    public function monthlyAttendance(Request $request)
    {
        if (\Auth::user()->can('manage report')) {
            ...
            $employeesAttendance = [];
            $totalPresent = $totalLeave = $totalEarlyLeave=$totalHoliday = 0;
            $ovetimeHours = $overtimeMins = $earlyleaveHours = $earlyleaveMins = $lateHours = $lateMins = 0;
        
            
            foreach ($employees as $id => $employee) {
                $attendances['name'] = $employee;
                $totalempPresent=$totalempHoliday=0;
                foreach ($dates as $date) {
                    $dateFormat = $year . '-' . $month . '-' . $date;

                    if ($dateFormat <= date('Y-m-d')) {
                        $resignation = Resignation::where('employee_id', '=' , $id)->first();
                      ...
                      if($resignation)
                      {
                        $check_leave_holiday=Holiday::where('date',$dateFormat)->Where('date','<=',$resignation->resignation_date)->first();
                      }else{
                        $check_leave_holiday=Holiday::where('date',$dateFormat)->first();
                      }
                        $check_month_leave = LeaveMapping::where('leave_date', $dateFormat)
                        ...
                        if(!empty($check_leave_holiday))
                        {
                            ...
                            $totalHoliday += 1;
                            $totalempPresent +=1;
                        }else if (!empty($employeeAttendance) ) {
                            if($employeeAttendance->day_count=='1')
                            {
                                $attendanceStatus[$date] = 'P';
                                $totalPresent += 1;
                                $totalempPresent += 1;
                            }else if($employeeAttendance->day_count=='0.5')
                            {
                                $attendanceStatus[$date] = 'half_day';
                                $totalPresent += 0.5;
                                $totalempPresent += 0.5;
                                
                            }else if($employeeAttendance->day_count=='0.25')
                            { 
                                 $attendanceStatus[$date] = 'two_hours_leave';
                                $totalPresent += 0.25;
                                $totalempPresent += 0.25;
                               

                            }else if($employeeAttendance->day_count=='0.75')
                            { 
                                 $attendanceStatus[$date] = 'two_hours_leave';
                                $totalPresent += 0.75;
                                $totalempPresent += 0.75;
                               

                            }else if(!empty($check_month_leave))
                            {
                ...
                $attendances['totalempPresent']=$totalempPresent;
                $attendances['totalempHoliday']=$totalempHoliday;
    ...
    public function payroll(Request $request)
    {
        ...
        if (\Auth::user()->can('manage report')) {
            ...
            $totalSalary = $totalGrossSalary = $totalNetSalary = $totalAllowance = $totalCommision = $totalLoan = $totalSaturationDeduction = $totalOtherPaymentAllowance=$totalOtherPaymentDeduction = $totalOverTime = 0;

            foreach ($payslips as $payslip) {
                $totalSalary += $payslip->total;
                $totalGrossSalary +=$payslip->gross_salary;
                $totalNetSalary += $payslip->net_payble;

             

                $saturationDeductions = json_decode($payslip->actual_saturation_deduction);
                ...
                foreach ($otherPayments as $otherPayment) {
                    if($otherPayment->payment_option=='allowance')
                    {
                        $totalOtherPaymentAllowance += $otherPayment->amount;
                    }else if($otherPayment->payment_option=='deduction')
                    {
                        $totalOtherPaymentDeduction +=$otherPayment->amount;
                    }
            ...
            $filterData['totalSalary'] = $totalSalary;
            $filterData['totalGrossSalary']=$totalGrossSalary;
            $filterData['totalNetSalary'] = $totalNetSalary;
          
         
            $filterData['totalSaturationDeduction'] = $totalSaturationDeduction;
            $filterData['totalOtherPaymentAllowance'] = $totalOtherPaymentAllowance;
            $filterData['totalOtherPaymentDeduction']=$totalOtherPaymentDeduction;
    ...
    public function getPayrollEmployee(Request $request)
    {
        if (!$request->department_id) {
            $employees = Employee::where('is_active','=','1')->get()->pluck('name', 'id')->toArray();
        } else {
            $employees = Employee::where('is_active','=','1')->where('department_id', $request->department_id)->get()->pluck('name', 'id')->toArray();
        }
...
//            -----------------------------
    ...
    public function salesReport(Request $request)
    {
        ...
        $invoiceCustomeres = Invoice::select('customers.name', \DB::raw('count(DISTINCT invoices.customer_id, invoice_products.invoice_id) as invoice_count'))
            ...
            ->selectRaw('(SELECT tax FROM invoice_products
             WHERE invoice_products.invoice_id = invoices.id) as tax')
    ...
    public function ReceivablesReport(Request $request)
    {
        ...
        foreach ($agingSummary as $item) {
            $name = $item["name"];
            $price = floatval(($item["price"] + $item['tax']) - ($item['pay_price'] + $item['credit_price']));
    ...
    public function PayablesReport(Request $request)
    {
        ...
        $payableVendors = Bill::select('venders.name')
            ...
            ->selectRaw('(SELECT tax FROM bill_products
        
         WHERE bill_products.bill_id = bills.id) as tax')
