        ...
        if (Auth::check()) {
            ...
            if (\Auth::user()->can('show hrm dashboard')) {
                ...
                if ($user->type != 'client') {
                    ...
                    $todayMonthDay = date('m-d'); // Get the current month and day in 'm-d' format
                    $birthday = Employee::where('is_active', '1')
                    ->whereRaw('DATE_FORMAT(dob, "%m-%d") >= ?', [$todayMonthDay])
                        ->orderByRaw('MONTH(dob), DAY(dob)')
                        ->take(5)
                        ->get();

                    $todays_leave = LeaveMapping::where('leave_date', date('Y-m-d'))
                    ...
                    $joining_date = Employee::where('is_active', '1')
                                    ->whereRaw('DATE_FORMAT(company_doj, "%m-%d") = ?', [$todayMonthDay])
                                    ->get();
                     $notice_period=Resignation::where('resignation_date','>=',date('Y-m-d'))
                     ...
                     $date = new DateTime();
                    $date->sub(new DateInterval('P3M')); // Subtract 3 months
                    $three_month_before_date=$date->format('Y-m-d'); // Output the date in 'YYYY-MM-DD' format
                   
                    $probation_date=Employee::where('is_active','1')
                     ->where('company_doj','>=',$three_month_before_date)
                     ->get();
                    ...
                    return view('dashboard.dashboard', compact('emp','birthday','todays_leave','joining_date','notice_period','probation_date','settings', 'announcements', 'employees', 'meetings', 'employeeAttendance', 'officeTime'));
                    ...
                    return view('dashboard.dashboard', compact('emp','birthday','todays_leave','joining_date','notice_period','probation_date','settings','arrEvents', 'onGoingTraining', 'activeJob', 'inActiveJOb', 'doneTraining', 'announcements', 'employees', 'meetings', 'countTrainer', 'countClient', 'countUser', 'notClockIns'));
                }
            } else {
