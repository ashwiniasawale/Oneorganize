...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        $revenue = Revenue::whereRaw('Date(date) = CURDATE()')->sum('amount');
    ...
    {
        $payment = Payment::whereRaw('Date(date) = CURDATE()')->sum('amount');
    ...
    {
        $currentMonth = date('m');
        $revenue = Revenue::whereRaw('MONTH(date) = ?', [$currentMonth])->sum('amount');
    ...
    public function expenseCurrentMonth()
    {
        ...
        $payment = Payment::whereRaw('MONTH(date) = ?', [$currentMonth])->sum('amount');
    ...
    public function getincExpBarChartData()
    {
        ...
        $user_id = \Auth::user()->creatorId();
        for ($i = 1; $i <= 12; $i++) {
            $monthlyIncome = Revenue::selectRaw('sum(amount) amount')->whereRaw('year(`date`) = ?', array(date('Y')))->whereRaw('month(`date`) = ?', $i)->first();
            ...
            $monthlyExpense = Payment::selectRaw('sum(amount) amount')->whereRaw('year(`date`) = ?', array(date('Y')))->whereRaw('month(`date`) = ?', $i)->first();
    ...
    public function getIncExpLineChartDate()
    {
        ...
        $dataArr['day'] = $arrDateFormat;
        for ($i = 0; $i < count($arrDate); $i++) {
            $dayIncome = Revenue::selectRaw('sum(amount) amount')->whereRaw('date = ?', $arrDate[$i])->first();
            ...
            $dayExpense = Payment::selectRaw('sum(amount) amount')->whereRaw('date = ?', $arrDate[$i])->first();
    ...
    public function invoicesData($start, $current)
    {
        $InvoiceProducts = Invoice::select('invoices.invoice_id as invoice')
            ...
            ->selectRaw('(SELECT SUM((price * quantity - discount+tax)) FROM invoice_products
    ...
    public function billsData($start, $current)
    {
        ...
        foreach ($mergedArray as $bill) {
            ...
            $billTotal += $bill['price'] + $bill['tax'] ;
            $billPaid += $bill['pay_price'] + $bill['debit_price'];
            $billDue += ($bill['price'] + $bill['tax'] ) - $bill['pay_price'] - $bill['debit_price'];
        }
       // dd($billTotal);
    ...
    public static function employeeIdFormat($number)
    {
       ...
       return trim($settings["employee_prefix"].$number);
    }
