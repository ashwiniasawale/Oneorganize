...
<div class="modal-body">
    <div class="row">
            ...
            <select name="chart_account_id" class="form-control" required="required">
                @foreach ($chartAccounts as $key => $chartAccount)
                    <option value="{{ $key }}" class="subAccount"
                        {{ $key == $bankAccount->chart_account_id ? 'selected' : '' }}>{{ $chartAccount }}</option>
                    @foreach ($subAccounts as $subAccount)
                        @if ($key == $subAccount['account'])
                            <option value="{{ $subAccount['id'] }}" class="ms-5"
                                {{ $subAccount['id'] == $bankAccount->chart_account_id ? 'selected' : '' }}> &nbsp;
        ...
        <div class="form-group  col-md-6">
            ...
            {{ Form::number('account_number', null, ['class' => 'form-control', 'required' => 'required','minlength' => '16', 'maxlength' => '18','id' => 'phoneInput']) }}
        </div>
        <div class="form-group  col-md-6">
        ...
        <div class="form-group  col-md-12">
            ...
            {{ Form::textarea('bank_address', null, ['class' => 'form-control', 'rows' => 3]) }}
        </div>
