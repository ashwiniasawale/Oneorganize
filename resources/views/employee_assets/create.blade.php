{{ Form::open(['url' => 'employee_assets_store', 'method' => 'post','id'=>'create_employee_assets']) }}

<div class="modal-body">
  <div class="row">
  <div class="form-group col-md-6">
  {{ Form::label('employee_id', __('Employee'),['class'=>'form-label']) }}
            {{ Form::select('employee_id', $employee,null, array('class' => 'form-control select2','required'=>'required')) }}
          </div>
        ...
        <div class="asset-entry row">
            <div class="form-group col-md-6">
                ...
                <select id="asset_id" name="asset_id[]" class="form-control select2" required>
                    ...
                    {
                        ?>
                        <option value="{{$a->id}}">{{$a->name}}({{$a->serial_number}})</option>
                        <?php
                    }
                }?>
                </select>
...
<div class="modal-footer">
    ...
    <input type="submit" id="employee_asset_submit" value="{{ __('Create') }}" class="btn btn-primary">
</div>
...
<script>
    ...
    function addAssetEntry() {
        ...
        newEntry.innerHTML = `
            <div class="form-group col-md-6">
                {{ Form::label('asset_id${i}', __('Asset Name'), ['class'=> 'form-label']) }}
