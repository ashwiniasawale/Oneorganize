{{Form::open(array('url'=>'attendanceemployee','method'=>'post','id'=>'mark_id'))}}
<div class="modal-body">
    <div class="row">
       
        <div class="form-group col-lg-6 col-md-6 ">
        <label class="form-label">{{__('Mark Attendance By')}}</label> <br>

        <div class="form-check form-check-inline form-group">
            <input type="radio" id="month" value="month" name="type" class="form-check-input" checked required>
            <label class="form-check-label" for="month">{{__('Month')}}</label>
        </div>
        <div class="form-check form-check-inline form-group">
            <input type="radio" id="daily" value="daily" name="type" class="form-check-input" >
            <label class="form-check-label" for="daily">{{__('Date')}}</label>
        </div>
        </div>
        <div class="form-group col-lg-6 col-md-6">
        ...
        <div class="form-group col-lg-6 col-md-6 months">
            <div class="btn-box">
                ...
                {{Form::month('month_date',date('Y-m'),array('class'=>'month-btn form-control month-btn','id'=>'months','required'=>'required'))}}
            </div>
        </div>
        <div class="form-group col-lg-6 col-md-6 dates">
            <div class="btn-box">
                {{ Form::label('date', __('Date'),['class'=>'form-label'])}}
                {{ Form::date('date',date('Y-m-d'), array('class' => 'form-control month-btn','id'=>'dates','required'=>'required')) }}
            </div>
        </div>
