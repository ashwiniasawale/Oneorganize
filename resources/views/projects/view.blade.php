...
<div class="row ">
    <div class="float-end">
        @can('share project')
            <a href="#" class="btn btn-sm btn-primary p-2" data-ajax-popup="true" data-size="md" data-title="{{ __('Shared Project Settings') }}"
...
{{--            <a href="#" id="{{ route('projects.link', \Illuminate\Support\Facades\Crypt::encrypt($project->id)) }}" class="btn btn-sm btn-primary btn-icon m-1 my-sm-2"--}}
{{--               onclick="copyToClipboard(this)" data-bs-toggle="tooltip" title="{{__('Click to copy link')}}">--}}
{{--                <i class="ti ti-link text-white"></i>--}}
{{--            </a>--}}
        @endcan
        @can('view project risk')
            <a href="{{ route('project.riskindex',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Risk')}}
            </a>
        @endcan
        @can('create milestone')
            
            <a href="#" data-size="lg" data-url="{{ route('project.milestone.create', $project->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="" class="btn btn-sm btn-primary my-1" data-bs-original-title="{{__('Create Milestone')}}">
                                   {{__('Milestones')}}
                                </a>
        @endcan
        @can('view communication record')
           <a href="{{ route('project.communication.index',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Communication Record')}}
            </a>
        @endcan
        @can('view requirement matrix')
            <a href="{{ route('project.requirementmatrix',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Requirement Matrix')}}
            </a>
        @endcan
        @can('view project task')
            <a href="{{ route('projects.gantt',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('WBS')}}
            </a>
        @endcan
        <!-- @if(\Auth::user()->type!='client' || (\Auth::user()->type=='client' ))
            <a href="{{ route('projecttime.tracker',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Tracker')}}
            </a>
        @endif
        @can('view expense')
            <a href="{{ route('projects.expenses.index',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Expense')}}
            </a>
        @endcan -->
        @if(\Auth::user()->type != 'client')
            @can('view timesheet')
                <a href="{{ route('timesheet.index',$project->id) }}" class="btn btn-sm btn-primary m-1">
                    {{__('Timesheet')}}
                </a>
            @endcan
        @endif
        @can('manage bug report')
            <a href="{{ route('task.bug',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Bug Report')}}
            </a>
        @endcan
        @can('manage project test')
            <a href="{{ route('project.testindex',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Test')}}
            </a>
        @endcan
        <!-- @can('create project task')
            <a href="{{ route('projects.tasks.index',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Task')}}
            </a>
        @endcan -->
        @can('view project review')
            <a href="{{ route('project.review.index',$project->id) }}" class="btn btn-sm btn-primary m-1">
                {{__('Review')}}
            </a>
        @endcan
      
        @can('edit project')
            <a href="#" data-size="lg" data-url="{{ route('projects.edit', $project->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Edit Project')}}" class="btn btn-sm btn-primary p-2 m-1">
                    ...
                    <div class="row align-items-center justify-content-between mt-10">
                        <div class="col-auto mb-3 mb-sm-0">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="ms-3">
                                    <small class="text-muted h6">{{__('Delay Task')}}</small>
                                    <h6 class="m-0">{{$project_data['task']['delay'] }}</h6>
                                </div>
                             
                            </div>
                        </div>
                        <div class="col-auto text-end">
                            <small class="text-muted h6">{{__('Inprogress Task')}}</small>
                            <h4 class="m-0">{{ $project_data['task']['inprogress'] }}</h4>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row align-items-center justify-content-between">
                        ...
                        <div class="col-auto text-end">
                            <h4 class="m-0">₹ {{$project->budget}}</h4>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        @can('edit project')
                            <div class="float-end">
                                <a href="#" data-size="xl" data-url="{{ route('invite.project.member.view', $project->id) }}" data-ajax-popup="true" data-title="{{__('Add Member')}}" title="Add Member" class="btn btn-sm btn-primary" data-bs-original-title="{{__('Add Member')}}">
        ...
        <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        @can('create milestone')
                            <div class="">
                                <a href="#" data-size="lg" data-url="{{ route('project.milestone.create', $project->id) }}" title="Create Milestone" data-title="{{__('Create Milestone')}}" data-ajax-popup="true"  class="btn btn-sm btn-primary" data-bs-original-title="{{__('Create Milestone')}}">
                                    <i class="ti ti-plus"></i>
                                </a>
                            </div>
                        @endcan
                                    ...
                                    <div class="row align-items-center justify-content-between">
                                        ...
                                        <div class="col-sm-auto text-sm-end d-flex align-items-center">
                                            @can('view milestone')
                                                ...
                                                <a href="#" data-size="lg" data-url="{{ route('project.milestone.show',$milestone->id) }}" data-ajax-popup="true" title="{{__('View')}}" data-title="{{__('View')}}" class="btn btn-sm" aria-label="View" >
                                            ...
                                            @can('edit milestone')
                                                <div class="action-btn bg-info ms-2">
                                                    <a href="#" data-size="lg" data-url="{{ route('project.milestone.edit',$milestone->id) }}" data-ajax-popup="true"  title="{{__('Edit')}}" data-title="{{__('Edit Milestone')}}" class="btn btn-sm">
                                            ...
                                            @can('delete milestone')
                                                <div class="action-btn bg-danger ms-2">
                                                    {!! Form::open(['method' => 'DELETE', 'class'=>'d-flex m-1','route' => ['project.milestone.destroy', $milestone->id]]) !!}
                                                    <a href="#" class="mx-3 btn btn-sm  align-items-center bs-pass-para"  title="{{__('Delete')}}" data-title="{{__('Delete')}}"><i class="ti ti-trash text-white"></i></a>
