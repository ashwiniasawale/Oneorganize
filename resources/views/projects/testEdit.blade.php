...
{{ Form::model($test, ['route' => ['project.test.update',[$project->id, $test->id]],'enctype' => 'multipart/form-data', 'id' => 'edit_test', 'method' => 'POST']) }}
<style>
    .choices__inner{
...
<div class="modal-body">
    ...
    <input type="hidden" id="p_end_date" value="<?php echo $project->end_date; ?>">
        <div class="form-group col-md-6">
            {{ Form::label('test_name', __('Test Name'),['class'=>'form-label']) }}
            {{ Form::text('test_name', $test->test_name, array('class' => 'form-control','required'=>'required')) }}
        </div>
        <div class="col-6">
            <div class="form-group">
                ...
                <option value="0" class="text-muted">{{__('Select Milestone')}}</option>
                    @foreach($project->milestones as $m_val)
                        <option value="{{ $m_val->id }}" {{ ($test->milestone_id == $m_val->id) ? 'selected':'' }}>{{ $m_val->title }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="col-12">
        <div class="form-group">
                {{ Form::label('test_description', __('Test Description'),['class' => 'form-label']) }}
                <small class="form-text text-muted mb-2 mt-0">{{__('This textarea will autosize while you type')}}</small>
                {{ Form::textarea('test_description', null, ['class' => 'form-control','rows'=>'2','data-toggle' => 'autosize']) }}
            </div>
</div>
<div class="col-6">
<div class="form-group">
...
<div class="form-group">
                ...
                {{ Form::date('start_date', null, ['class' => 'form-control','required'=>'required','id'=>'start_date','onchange'=>'getdate(this.value);']) }}
            </div>
</div>
<div class="col-6">
<div class="form-group">
        ...
        <div class="form-group  col-md-6">
            {{ Form::label('test_procedures', __('Test Procedures'),['class'=>'form-label']) }}
            {!! Form::textarea('test_procedures', null, ['class'=>'form-control','rows'=>'2']) !!}
        </div>
        <div class="form-group  col-md-6">
            {{ Form::label('test_input', __('Test Input'),['class'=>'form-label']) }}
            {!! Form::textarea('test_input', null, ['class'=>'form-control','rows'=>'2']) !!}
        </div>
        <div class="form-group  col-md-6">
            {{ Form::label('test_accepted_output', __('Test  Accepted Output'),['class'=>'form-label']) }}
            {!! Form::textarea('test_accepted_output', null, ['class'=>'form-control','rows'=>'2']) !!}
        </div>
       

        <div class="form-group  col-md-12">
            {{ Form::label('test_note', __('Test Note'),['class'=>'form-label']) }}
            {!! Form::textarea('test_note', null, ['class'=>'form-control','rows'=>'2']) !!}
        </div>
        <div class="form-group col-md-6">
            {{Form::label('test_plan',__('Test Plan'),['class'=>'form-label'])}}<span class="text-danger">*</span>
        ...
        <div class="form-group col-md-6">
            {{Form::label('test_result',__('Test Result'),['class'=>'form-label'])}}<span class="text-danger">*</span>
            <div class="form-file mb-3">
        ...
        <div class="form-group col-md-6">
            {{ Form::label('test_type', __('Test Type'),['class'=>'form-label']) }}
