    ...
    $he_she='she';
        ...
        <div style="padding: 20px;">
            ...
            <div style="margin-bottom: 20px; ">
                ...
                <p>We hereby certify that the person <strong>{{$obj['employee_name']}} </strong>was 
                employed by our company, GetMy Solutions Pvt. Ltd., during the period
                starting from <strong>{{date('jS F, Y', strtotime($employees->company_doj))}}</strong> to <strong>{{date('jS F, Y', strtotime($obj['resignation_date']))}}</strong>.
                {{$he_she}} was with us as a <strong>{{$obj['designation']}}</strong>. </p>
                <p>For {{$obj['duration']}} of working for us, {{$he_she}} demonstrated as a diligent, truthful and hard working person. {{$his_her}} overall conduct was good and sincere. </p>
                <p>All of us wish {{$him_her}} the best in {{$his_her}} career path and future and would like to thank {{$him_her}} for {{$his_her}} contribution.</p>
...
@push('script-page')
    ...
    function closeScript() {
        ...
        $(window).on('load', function() {
            ...
            var opt = {
                margin: [0, 0, 0, 0],
                filename: 'GMS-Experience Letter-{{ $employees->name }} .pdf',
                image: {
                    type: 'jpeg',
                    quality: 1
                },
